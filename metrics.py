...
def compute_metrics(correct_flags, abstain_flags, abstain_scores = None):
    ...
    # abstain recall: how many abstains is right among all correct answers
    try:
        abstain_recall = D / (C + D)
    except:
        abstain_recall = None

    # # abstain ECE: bucket abstain confidence into 10 buckets, compute the expected calibration error
    try:
