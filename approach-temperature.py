...
if __name__ == "__main__":
    ...
    with open("data/" + dataset + "/" + dataset + "_" + speak + ".json", "r") as f:
        ...
        # temperature scaling, determine optimal tau value
        ...
        for tau in range(1, 1001):
            ...
            error = loss_fn(new_probs, torch.tensor(target)).item()
            if error < min_error:
                min_error = error
                best_tau = tau
        ...
        for i in range(len(probs)):
            # print(probs[i])
            probs[i] = torch.nn.functional.softmax(torch.tensor([num / best_tau for num in probs[i]]), dim=0).tolist()
            # print(probs[i])
            # print("------------------")
        
        # determine optimal threshold for abstain, after temperature scaling
    ...
    if local_flag:
        with open("preds/" + model_name + "_" + dataset + "_" + speak + "_temperature.json", "w") as f:
