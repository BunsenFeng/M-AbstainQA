...
if __name__ == "__main__":
    ...
    with open("data/" + dataset + "/" + dataset + "_" + speak + ".json", "r") as f:
        ...
        if approach_type == "self": # expert reviewers
            for prompt_feedback in tqdm(prompt_feedback_list):
                ...
                for k in range(3):
                    ...
                    prompt_feedback_experts.append(prompt_feedback.replace("<language>", specified_language))
        ...
        if feedback_flag:
            ...
            with open("feedbacks/" + model_name + "_" + dataset + "_" + speak + "_" + approach_type + "_multirelated.json", "w") as f:
                json.dump(generated_feedback_list, f, indent=4)

    # print(abstain_scores)

    if local_flag:
        with open("preds/" + model_name + "_" + dataset + "_" + speak + "_multirelated.json", "w") as f:
            json.dump({"correct_flags": correct_flags, "abstain_flags": abstain_flags, "abstain_scores": abstain_scores}, f)
        
    print("------------------")
    print("Approach: multirelated")
