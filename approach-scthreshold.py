...
if __name__ == "__main__":
    ...
    with open("data/" + dataset + "/" + dataset + "_" + speak + ".json", "r") as f:
        ...
        for d in tqdm(data["test"]):
            ...
            answers = []
            for k in range(path):
                response = lm_utils.llm_response(original_prompt, model_name, probs=False, temperature=1, max_new_tokens=5)
                answers.append(lm_utils.answer_parsing(response))
            count_largest = 0
            key_largest = None
            for key in d["choices"].keys():
    ...
    if local_flag:
        with open("preds/" + model_name + "_" + dataset + "_" + speak + "_scthreshold.json", "w") as f:
